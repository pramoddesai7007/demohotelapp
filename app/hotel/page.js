...
const CreateHotelForm = () => {
  ...
  const fetchHotels = async () => {
    try {
      const response = await axios.get('https://demoback-one.vercel.app/api/hotel/get-all');
  ...
  const handleEdit = async (hotelId) => {
    try {
      ...
      const response = await axios.get(`https://demoback-one.vercel.app/api/hotel/get/${hotelId}`);
...
const handleEditInputChange = (e) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`https://demoback-one.vercel.app/api/hotel/edit/${editFormData._id}`, formDataForUpload);
  ...
  //   try {
  //     await axios.delete(`https://demoback-one.vercel.app/api/hotel/delete/${hotelId}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`https://demoback-one.vercel.app/api/hotel/delete/${deleteHotelId}`);
      console.log('Hotel deleted successfully.');
      fetchHotels(); // Refresh the list after deletion
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://demoback-one.vercel.app/api/hotel/create', formDataForUpload);
              ...
              <tbody>
                {hotels.map((hotel) => (
                  <tr
                    ...
                    {hotel.hotelLogo && <img src={`https://demoback-one.vercel.app/${hotel.hotelLogo}`} alt="Hotel Logo" className="w-10 h-10" />}
                    </td>
